...
static bool __init_qspi_bus()
{
    ...
    ESP_LOGI(TAG, "SPI bus OK, spi=%p", spi);
...
void amoled_write_cmd(uint32_t cmd, uint8_t *pdat, uint32_t lenght)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "write_cmd FAIL addr=0x%04lx err=%d", cmd, ret);
    }
    clrCS();
}
...
void amoled_push_buffer(uint16_t *data, uint32_t len)
{
    ...
    do {
        ...
        esp_err_t ret = spi_device_polling_transmit(spi, (spi_transaction_t *)&t);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "push_buffer FAIL err=%d", ret);
        }
        len -= chunk_size;
        p += chunk_size;
    } while (len > 0);
